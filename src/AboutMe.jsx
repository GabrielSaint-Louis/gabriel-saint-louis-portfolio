...
import reveal1 from "./assets/1.png";
import reveal2 from "./assets/2.png";
import reveal3 from "./assets/3.png";
...
const REVEAL_CONTENT = [
  {
    upper: ["Gabriel Saint-Louis", "age: 20"],
    lower: "major: computer science & project management",
  },
  {
    upper: [
      "Work-study @ AXONE CONSEILS",
      "Building a payroll management software",
      "Design, architecture & implementation from scratch",
    ],
    lower: "currently on a work-study contract",
  },
  {
    upper: [
      "Dev studio @ Ambriel — mobile apps & websites",
      "Web & mobile projects for businesses and freelancers",
      "From concept to deployment, end to end",
    ],
    lower: "open for new projects",
    link: "https://ambriel.fr",
    linkLabel: "ambriel.fr",
  },
];
import char1 from "./assets/newchar3.png";
import char2 from "./assets/newchar2.png";
import char3 from "./assets/newchar1.png";

const CHARS = [char1, char2, char3];

const ROLES = [
  { text: "I'M",  color: "#e8c100", bg: "rgba(232,193,0,0.12)", border: "rgba(232,193,0,0.5)" },
  { text: "THE",  color: "#4a8fff", bg: "rgba(74,143,255,0.12)", border: "rgba(74,143,255,0.5)" },
  { text: "BEST", color: "#4a8fff", bg: "rgba(74,143,255,0.12)", border: "rgba(74,143,255,0.5)" },
];
...
const ITEMS = [
  ...
  {
    id: "instagram", label: "INTERNSHIP", handle: "@yourhandle", href: "https://instagram.com/yourhandle", icon: "📷", barIcon: icon2, bars: 5, newBars: [1, 2], counts: ["3.4M", "2.5M", "676K", "412K", "198K"],
  ...
  {
    id: "tiktok", label: "FREELANCE", handle: "@yourhandle", href: "https://tiktok.com/@yourhandle", icon: "🎵", barIcon: icon3, bars: 7, newBars: [0, 3, 5, 6], counts: ["5.1M", "3.7M", "2.2M", "1.4M", "831K", "490K", "217K"],
...
export default function AboutMe() {
  ...
  useEffect(() => {
    const onKey = (e) => {
      ...
      if (e.key === "Enter") { playClickSound(); setRevealed(true); }
  ...
  return (
    <div id="menu-screen">
      ...
      {revealed && (
        <div key={`panel-${active}`} className={`sc-reveal-panel${mounted ? " mounted" : ""}`}>
          ...
          <div className="sc-reveal-lower-bar">
            {REVEAL_CONTENT[active].link ? (
              <a
                href={REVEAL_CONTENT[active].link}
                target="_blank"
                rel="noreferrer"
                style={{ color: "#db3400", textDecoration: "none", fontFamily: "inherit", fontWeight: "inherit", fontSize: "inherit", letterSpacing: "inherit" }}
              >
                {REVEAL_CONTENT[active].linkLabel}
              </a>
      ...
      {revealed && (
        <div key={`portrait-${active}`} className={`sc-main-portrait-shell${mounted ? " mounted" : ""}`} style={{ background: ['#3d1200', '#db3400', '#efb85d'][active] }}>
          <img
            className="sc-main-portrait"
            src={MAIN_IMAGES[active]}
            alt=""
            style={active === 2 ? { objectPosition: 'center bottom' } : {}}
          />
        </div>
      )}
      <style>{`
        ...
        .sc-reveal-panel {
          ...
          left: -6vw;
          width: 88vw;
          height: min(60vh, calc(100vh - 44vh - 4vh));
          z-index: 12;
          pointer-events: none;
          background:
        ...
        @media (max-width: 768px) {
          .sc-bar { width: 80vw; height: 52px; }
          .sc-bar-outer.active .sc-bar,
          .sc-bar-outer.active .sc-bar-red { height: 70px; }
          .sc-bar-red { width: 80vw; }
          .sc-role { font-size: 34px; padding: 0 10px 0 4px; }
          .sc-label { font-size: 20px; letter-spacing: 2px; }
          .sc-char { left: 72px; max-width: 60px; }
          .sc-main { padding-left: 48px; }
          .sc-stat-num { font-size: 18px; }
          .sc-stats { padding-right: 10px; gap: 6px; }
          .sc-main-portrait-shell { width: 58vw; right: -4vw; }
          .sc-reveal-panel { top: 50vh; left: -3vw; width: 94vw; height: 52vh; }
          .sc-reveal-upper-line { font-size: 14px; }
          .sc-reveal-lower-bar { font-size: 15px; padding-left: 14px; }
          .sc-right-nav { top: 6vh; left: 3vw; }
          .sc-right-nav .sc-nav-btn { font-size: 70px; }
          .sc-footer { bottom: 10px; right: 12px; }
        }
        @media (max-width: 480px) {
          ...
          .sc-main-portrait-shell { width: 65vw; }
          .sc-reveal-panel { top: 54vh; left: -2vw; width: 98vw; }
          .sc-reveal-upper-line { font-size: 12px; }
          .sc-right-nav .sc-nav-btn { font-size: 54px; }
        }
      `}</style>
